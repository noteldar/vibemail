...
class ConversationStarter(BaseModel):
    ...
    user_interest_level: (
        str  # How interested the user seemed in this topic (High/Medium/Low)
    )
    ...
    async def deep_research(ctx: RunContext[StarterGeneratorDeps], query: str) -> str:
        ...
        try:
            ...
            if search_results and "data" in search_results:
                ...
                research_summary += (
                    f"- Timely information that would interest users\n\n"
                )
                research_summary += f"CONVERSATION STARTER POTENTIAL:\n"
                research_summary += (
                    f"- Use surprising facts or statistics from these sources\n"
                )
                research_summary += (
                    f"- Reference recent developments to show currency\n"
                )
                research_summary += f"- Connect findings to user's previous interests\n"
                research_summary += (
                    f"- Create curiosity gaps with partial information\n"
                )
